...
export class Migrator {
  ...
  async initialize() {
    try {
      ...
      throw new QueryError('Failed to create migrations table', 'CREATE TABLE migrations');
    }
  }
  ...
  parseMigration(content: string): ParsedMigration {
    ...
    const up = content
      .substring(upIndex + UP_MARKER.length, downIndex)
      .trim();
    
    const down = content
      .substring(downIndex + DOWN_MARKER.length)
      .trim();
  ...
  async runMigration(filename: string, direction: 'up' | 'down' = 'up') {
    ...
    try {
      ...
      throw new TransactionError(`Failed to ${direction} migration: ${filename}`, { error });
    }
  }
