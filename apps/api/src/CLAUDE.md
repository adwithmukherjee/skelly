...
// 3. Create handler factory function
export function createCreateResourceHandler(deps: ResourceControllerDeps) {
  return createHandler(
    {
      body: createResourceSchema, // Validation schema
    },
    async ({ body, logger }) => {
      // Business logic
...
3. **Share common schemas**:
   ```typescript
   // Import shared schemas instead of duplicating
   import { getUserSchema } from './getUser';
   
   export const updateUserSchema = z.object({
     params: getUserSchema, // Reuse ID validation
     body: z.object({...})
   });
   ```
